...
pub trait CommandUp {
    ...
    fn up_all<CR>(&self, runner: &CR, ovr: &Override) ->
        Result<(), Error>
        where CR: CommandRunner;

    /// Up all the images in the specified pods.
    fn up<CR>(&self, runner: &CR, ovr: &Override, pod_names: &[&str]) ->
        Result<(), Error>
        where CR: CommandRunner;
}
...
impl CommandUp for Project {
    ...
    fn up<CR>(&self, runner: &CR, ovr: &Override, pods_names: &[&str]) ->
        Result<(), Error>
        where CR: CommandRunner
    {
        for pod_name in pods_names {
            let pod = try!(self.pod(pod_name).ok_or_else(|| {
...
fn runs_docker_compose_up_on_specified_pods() {
    ...
    assert_ran!(runner, {
        ["docker-compose",
         "-p", "rails_hello",
         "-f", proj.output_dir().join("pods/db.yml"),
         "-f", proj.output_dir().join("pods/overrides/development/db.yml"),
         "up", "-d"]
    });
    proj.remove_test_output().unwrap();
}
